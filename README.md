...
이 프로그램은 Hammerspoon 위에서 실행되는 경량 스크립트입니다.

| 항목 | 스크립트 자체 | Hammerspoon 포함 |
|------|-------------|------------------|
| 메모리 | ~1-2MB | ~30-80MB |
| CPU (유휴) | ~0% | ~0.1-0.5% |
| CPU (타이핑 중) | ~0.1-0.5% | ~0.5-2% |

### 자원 사용 요소

- **메모리**: Canvas UI, eventtap 리스너, 타이머 3개
- **CPU**: 키 입력마다 카운트 증가, 100타/30초마다 파일 저장
- **디스크**: today_count.txt (수십 바이트), keystroke_log.txt (하루 1회)

> 참고: 일반 웹 브라우저 탭 하나(~100-300MB) 대비 매우 경량입니다.
